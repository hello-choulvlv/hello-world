...
private:
	...
	void   initWithTarget(SceneTracer *eventTarget);
	void   initWithTarget(SceneTracer *eventTarget, GLKTouchCallback touchBegin, GLKTouchMotionCallback touchMoved, GLKTouchReleaseCallback touchRelease);

	//派发事件
	bool    onTouchBegin(const float_2 &touchPoint);
	void    onTouchMoved(const float_2 &touchPoint);
	void    onTouchEnded(const float_2 &touchPoint);
public:
	~TouchEventListener();
	static TouchEventListener *createTouchListener(SceneTracer *eventTarget);
	static TouchEventListener *createTouchListener(SceneTracer *eventTarget,GLKTouchCallback touchBegin, GLKTouchMotionCallback touchMoved, GLKTouchReleaseCallback touchRelease);
	...
	//获取事件派发者的地址
	SceneTracer *getEventTarget()const;
	//设置

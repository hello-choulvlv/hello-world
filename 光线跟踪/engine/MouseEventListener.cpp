...
void MouseEventListener::initWithTarget(SceneTracer *target, GLKMousePressCallback pressCallback, GLKMouseMotionCallback motionCallback, GLKMouseReleaseCallback releaseCallback)
{
	_eventTarget = target;
...
MouseEventListener *MouseEventListener::createMouseEventListener(SceneTracer *target, GLKMousePressCallback pressCallback, GLKMouseMotionCallback motionCallback, GLKMouseReleaseCallback releaseCallback)
{
...
void  MouseEventListener::onMousePressed( MouseType mouseType,const float_2 &position)
{
	_isResponseEvent = false;
	if (_mousePressCallback)
		_isResponseEvent=(_eventTarget->*_mousePressCallback)(mouseType,position);
}
void MouseEventListener::onMouseMoved( MouseType mouseType, const float_2 &position)
{
	if (_isResponseEvent && _mouseMoveCallback)
		(_eventTarget->*_mouseMoveCallback)(mouseType,position);
}

void MouseEventListener::onMouseReleased( MouseType mouseType, const float_2 &position)
{

...
// rendering method
int g_iRenderingMethod = METHOD_DX10_INST;
bool g_bUseSceneDependentProjection = true;
bool g_bMethodSupported[4] = {true, true, true, true};
...
int g_iShadowTrianglePerFrame = 0;
...
void PrintStats(char *strText)
{
  ...
  if(g_bHUDStats)
    _snprintf(strStats,iSize,"%s\n%i splits, texture type: %s\n%i objects, %ik/%dk tris per frame, %iMB texture memory\n", strMethod, g_iNumSplits, g_ShadowMaps[0]->GetInfoString(), g_iNumSceneObjects, g_iTrisPerFrame/1000, g_iShadowTrianglePerFrame/1000, iTexMem);
  if(g_bHUDFPS)

...
//
//
static float g_fOriginalCameraNear;
static float g_fOriginalCameraFar;
static void SetDepthRange(float fNear, float fFar)
{
  g_fOriginalCameraNear = g_Camera.m_fNear;
  g_fOriginalCameraFar = g_Camera.m_fFar;
...
{
  glDepthRange(0, 1);
  g_Camera.m_fNear = g_fOriginalCameraNear;
  g_Camera.m_fFar = g_fOriginalCameraFar;
  g_Camera.CalculateMatrices();
}
...
void Render_MP(void)
{
  ...
  for(int i = 0; i < g_iNumSplits; i++)
  {
    ...
    // render scene
	//注意glDepthRange函数的含义,将片元的深度截取到这个区间,位于这个区间之外的都会被丢弃掉

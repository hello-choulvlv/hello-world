...
static Matrix g_dx9mTextureMatrix[5];
...
static void RenderCasters(const std::vector<SceneObject *> &Objects, const Matrix &mViewProj)
{
  ...
  for(unsigned int j = 0; j < Objects.size(); j++)
  {
	...
	g_iShadowTrianglePerFrame += pObject->m_pMesh->m_iNumTris;
  }
}
...
void Render_DX9(void)
{
  ...
  for(int i = 0; i < g_iNumSplits; i++)
  {
    ...
    // find casters
	std::vector<SceneObject *> casters = g_Light.FindCasters(splitFrustum);

    // calculate crop matrix
	Matrix mCropMatrix = g_Light.CalculateCropMatrix(casters, receiver_boundingbox,splitFrustum);// receivers, splitFrustum);
    // calculate view-proj matrix
    Matrix mSplitViewProj = g_Light.m_mView * g_Light.m_mProj * mCropMatrix;
    // calculate texture matrix
	g_dx9mTextureMatrix[i] = mSplitViewProj * GetTexScaleBiasMatrix();
  ...
  // render scene
  SetShaderParams(g_dx9mTextureMatrix);

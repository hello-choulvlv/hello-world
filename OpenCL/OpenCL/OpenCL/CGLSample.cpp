...
void displayTexture(int w, int h)
{
	...
	glBindTexture(GL_TEXTURE_2D, tex);//GL_TEXTURE_RECTANGLE_ARB
	glBegin(GL_QUADS);
	glTexCoord2f(0, 0);
	glVertex2f(0, 0);
	glTexCoord2f(0, 1.0f);//h
	glVertex2f(0, h);
	glTexCoord2f(1.0f, 1.0f);//w,h
	glVertex2f(w, h);
	glTexCoord2f(1.0f, 0);//w
	glVertex2f(w, 0);
	glEnd();
	glDisable(GL_TEXTURE_2D);//GL_TEXTURE_RECTANGLE_ARB
}
...
void initTexture(int width, int height)
{
	...
	glBindTexture(GL_TEXTURE_2D, tex);//GL_TEXTURE_RECTANGLE_ARB
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F_ARB, width,height, 0, GL_LUMINANCE, GL_FLOAT, NULL);//GL_TEXTURE_RECTANGLE_ARB
}
...
cl_int computeTexture()
{
	...
	errNum = clEnqueueNDRangeKernel(commandQueue, tex_kernel, 2, NULL, tex_globalWorkSize, tex_localWorkSize,
		0, NULL, NULL);
	if (errNum != CL_SUCCESS)
	{
		std::cerr << "Error queuing kernel for execution." << std::endl;
...
void Cleanup()
{
	...
	if (tex != 0)
	{
		glBindBuffer(GL_TEXTURE_2D, tex);//GL_TEXTURE_RECTANGLE_ARB

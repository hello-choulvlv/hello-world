/*
  *点定位算法实现.
  *在实现的过程中,为了使算法看起来简单易懂且易调试,
  *我们并没有对代码进行优化,并且算法在内存管理上面比较粗放,读者可以尝试着自己去解决这些问题.
  *梯形图算法的核心思想相对比较简单,但是其实现过程因为牵涉到多种拓扑关系的重构,因此算法实现起来
  *比较困难.作者自己也是经过了各种痛苦过程才对这个过程有了深入的了解,因此在代码的实现过程中,对许多关键的地方
  *做了详细的注释,并且对主循环过程的每一步的计算过程都进行了严格的正确性验证,详细实现过程读者可以自己查看.
  *2020年4月13日
  *@author:xiaohuaxiong
 */

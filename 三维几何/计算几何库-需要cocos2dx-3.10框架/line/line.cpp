...
{
	float f = cross(line1.direction,line2.direction);
	if (fabsf(f) <= 0.001f)
		return false;
	float rx = line1.start_point.x - line2.start_point.x;
	float ry = line1.start_point.y - line2.start_point.y;
	//使用行列式算法
	auto &d1 = line1.direction;
	auto &d2 = line2.direction;
...
bool check_target_point_feasible(const Vec2 &target_point,const Line2D *lines_array,int array_size,int ax,int bx)
{
	for (int index_l = 0; index_l < array_size; ++index_l)
	{
		if (index_l != ax && index_l != bx)
		{
			const Line2D &line = lines_array[index_l];//<-y,x>
			float f = -line.direction.y * (target_point.x - line.start_point.x) + line.direction.x * (target_point.y - line.start_point.y);
			if (f < 0)
				return false;
		}
	}
...
int linearly_program_2d(const std::vector<Line2D> &lines, const cocos2d::Vec2 &target_coeff, cocos2d::Vec2 &intersect_point, cocos2d::Vec2 &direction)
{
	//首先判定目标线性规划是否无界
	float left_boundary = FLT_MAX, right_boundary = -FLT_MAX;
	//同时现需要记录目标函数的两个凭证半平面
	int h1 = -1,h2 = -1;
	const int array_size = lines.size();
	std::vector<int>   boundary_lines;
	for (int index_l = 0; index_l < array_size; ++index_l)
	{
		const Line2D &line = lines[index_l];
		//注意,一定要使用直线的左侧法线,这也是函数的输入约定(-y,x)
		float normal_x = -line.direction.y;
		float normal_y = line.direction.x;
		if (normal_x > 0)//左侧有界
			left_boundary = fmax(left_boundary, -normal_y / normal_x);
		else if (normal_x < 0)//右侧有界
			right_boundary = fmin(right_boundary, -normal_y / normal_x);
		else
			vector_fast_push_back(boundary_lines,index_l);
		if (normal_y < 0)//求直线的法线与+X轴的夹角
		{
			//左侧
			float f = cross(Vec2(normal_x,normal_y),Vec2(1,0));
			if (f >= 0)
				h1 = index_l;
			else
				h2 = index_l;
	...
	//因为目前本人还没有中找到一个线性时间复杂度的算法,可以快速找出一条直线与N条其他直线的交点
	//因此,先用一个 N^2级别的算法代替,到后面再改进.
	if (left_boundary <= right_boundary)//如果已经判定出原线性规划无界
	{
		//那么就求出可以无限增长的可行解区域内的射线,或者判定出原线性规划无解
		//求boundary_lines直线与x轴的交点,目前已知
		float left_x = -FLT_MAX;
		float right_x = FLT_MAX;
		for (int index_l = 0; index_l < boundary_lines.size(); ++index_l)
		{
			int line_index = boundary_lines[index_l];
			const Line2D &line = lines[index_l];
			float normal_x = -line.direction.y;//y分量必为0
			if (normal_x > 0)
				left_x = fmax(left_x, line.start_point.x);
			else
				right_x = fmin(right_x,line.start_point.x);
		}
		//如果与目标直线平行的约束条件彼此冲突
		bool whether_feasible = boundary_lines.size()>=2 ?left_x <= right_x:true;
		//如果是可行的,则给出一条射线
		if (!whether_feasible)
			return false;
		direction.x = (left_boundary + right_boundary) * 0.5f;
		direction.y = 1.0f;
		//任意一个可行的解,此处将会使用一个笨拙的算法
		float f = FLT_MAX;
		for (int index_l = 0; index_l < array_size - 1; ++index_l)
		{
			for (int index_t = index_l + 1; index_t < array_size; ++index_t)
			{
				//如果两直线平行,则任取直线上一点
				if (fabsf(cross(lines[index_l].direction, lines[index_t].direction)) <= 0.001f)
				{
					if (check_target_point_feasible(lines[index_l].start_point, lines.data(), array_size, index_l, -1))
					{
						f = fminf(f, lines[index_l].start_point.y);
						intersect_point = lines[index_l].start_point;
					}
				}
				else {
					line_line_intersect_point(lines[index_l], lines[index_t], intersect_point);
					//如果当前点是可行的,则检测其数值是否最小
					if (check_target_point_feasible(intersect_point, lines.data(), array_size, index_l, index_t))
						f = fminf(f, intersect_point.y);
				}
			}
		}
		return true;
	...
	assert(h1 != -1 && h2 !=-1);
	std::vector<Line2D>  temp_lines = lines;
	Line2D l1 = lines[h1];
	Line2D l2 = lines[h2];
	temp_lines.erase(temp_lines.begin() + (h1 > h2 ?h2:h1));
	temp_lines.erase(temp_lines.begin() + (h1 < h2?h1:h2));
	temp_lines.insert(temp_lines.begin(),l1);
	temp_lines.insert(temp_lines.begin(),l2);

	line_line_intersect_point(lines[h1],lines[h2],intersect_point);
	for (int index_l = 2; index_l < array_size; ++index_l)
	{
		auto &line = temp_lines[index_l];
		float normal_x = -line.direction.y;
		float normal_y = line.direction.x;
		//检测上次得出的交点是否也在新的约束条件范围之内
		float f = normal_x * (intersect_point.x - line.start_point.x) + normal_y * (intersect_point.y - line.start_point.y);
		if (f < 0)//此时需要重新计算新的交点
		{
			float t = -FLT_MAX;
			for (int index_y = 0; index_y < index_l; ++index_y)
			{
				auto &secondary_line = temp_lines[index_y];
				Vec2 target_point;
				line_line_intersect_point(line, secondary_line, target_point);
				if (check_target_point_feasible(intersect_point, temp_lines.data(), index_l + 1, index_l, index_y))
				{
					if (target_point.y > t)
					{
						t = target_point.y; intersect_point = target_point;
					}
				}
			}
			if (t == -FLT_MAX)

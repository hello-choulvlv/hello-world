...
void      WaterCaustic::init()
{
	...
	const int     formatType[1] = {GL_RGBA32F};
	...
	_normalTexture = RenderTexture::createRenderTexture(textureSize, RenderTexture::RenderType::ColorBuffer, formatType);
...
{
	_camera = Camera::createCamera(GLVector3(0.0f, WaterMesh / 1.2f, WaterMesh), GLVector3(0.0f, 0.0f,-WaterMesh / 1.2f), GLVector3(0.0f, 1.0f, 0.0f));
...
{
	_waterHeight = WaterMesh;
	_waterModelMatrix.translate(0, _waterHeight, -WaterMesh);
	_lightDirection = GLVector3(0.0f, WaterMesh * 3,0.0f);
	/*
	  *水底平面网格
	 */
	_groundHeight = -WaterMesh / 4.0f;

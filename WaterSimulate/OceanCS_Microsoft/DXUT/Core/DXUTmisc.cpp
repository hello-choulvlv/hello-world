...
#define BUFFER_LEN	256
	...
	if (wc_strFile)
	{
		swprintf_s(complete_msg, BUFFER_LEN, L"File: %s Line: %d HRESULT: %u \n %s", wc_strFile, dwLine, hr, strMsg);
		delete [] wc_strFile;
	}
	else
	{
		swprintf_s(complete_msg, BUFFER_LEN, L"HRESULT: %u \n %s", hr, strMsg);
	}

	::OutputDebugString(complete_msg);
	if (bPopMsgBox)
	{
		::MessageBox(NULL, (LPCWSTR)complete_msg, L"DXUT Message", MB_OK);
	}
